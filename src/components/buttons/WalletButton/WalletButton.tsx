...
function Button(props: Props) {
  ...
  const Class = `wallet ${props.col && props.col} ${props.color}`;

  const roughSpan = () => {
    let dataSpan = props.title.split("")
    return dataSpan.map((value:string) => {
      return (
        <span>{value}</span>
      )
    })
  ...
  return (
    <button className={Class} style={Style}>
      ...
      <svg className="wallet__svg" role="presentational" viewBox="0 0 600 600">
        <defs>
          <clipPath id="myClip">
